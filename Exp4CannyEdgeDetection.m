...
clear all;
%input image
img = imread('lena_color_256.tif');%color image
%show input image
%Figure 1
...
%Figure 2
subplot(222);imagesc(arah2);colorbar;title('Image after direction adjustment');
...
for i=2:pan-1
    for j=2:leb-1
        if(arah2(i,j)==0)
            BW(i,j)=(magnitude2(i,j) == max([magnitude2(i,j),magnitude2(i,j+1),magnitude(i,j-1)]));
        elseif(arah2(i,j)==45)
...
BW=BW.*magnitude2;
%Figure 3
subplot(223);imshow(BW);title('Non-Max Supression Image');
...
%Figure 4
subplot(224);imshow(edge_final);title('final edge image');